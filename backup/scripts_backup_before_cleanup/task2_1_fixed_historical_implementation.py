...
CONFIG = {
    ...
    "API_DELAY": 1.0,      # Increased delay between API calls to avoid rate limits
    "BATCH_SIZE": 50,      # Process options in batches for ltp() bulk quotes
    "SNAPSHOT_TIME": "15:25",  # EOD snapshot time
    "MAX_RETRIES": 5,      # Increased max retries for API calls
...
def fetch_option_chain_snapshot_historical(client, expiry_date: date, strike_range: range, trade_date: date) -> List[Dict]:
    ...
    try:
        ...
        # Step 3: Fetch historical data for each option
        option_records = []
        ...
        logger.info(f"✅ Successfully fetched historical data for {len(option_records)} options out of {total_opts}")
